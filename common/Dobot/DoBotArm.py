...
# sys.path.insert(1, './Dobot')
import common.Dobot.DobotDllType as dType
# from common.Dobot import DobotDllType as dType
...
class DoBotArm:
    def __init__(self, homeX, homeY, homeZ, homeR=0, portName="COM16"):
        self.suction = False
        self.picking = False
        self.api = dType.load()
        self.homeX = homeX
    ...
    def dobotConnect(self):
        if (self.connected):
            ...
            state = dType.ConnectDobot(self.api, self.portName, 115200)[0]
            if (state == dType.DobotConnect.DobotConnect_NoError):
                print("Connect status:", CON_STR[state])
                dType.SetQueuedCmdClear(self.api)

                dType.SetHOMEParams(self.api, self.homeX, self.homeY, self.homeZ, self.homeR, isQueued=1)
                dType.SetPTPJointParams(self.api, 200, 200, 200, 200, 200, 200, 200, 200, isQueued=1)
                dType.SetPTPCommonParams(self.api, 100, 100, isQueued=1)    # 速度 加速度
    ...
    def pickToggle(self, itemHeight):
        ...
        if (self.picking):
            lastIndex = \
            dType.SetPTPCmd(self.api, dType.PTPMode.PTPMOVLXYZMode, positions[0], positions[1], self.homeZ, 0)[0]
            self.picking = False
        else:
            lastIndex = \
            dType.SetPTPCmd(self.api, dType.PTPMode.PTPMOVLXYZMode, positions[0], positions[1], itemHeight, 0)[0]
            self.picking = True
        self.commandDelay(lastIndex)
